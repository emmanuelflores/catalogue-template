...
-(void) enableShadow {
    ...
    self.layer.shadowOffset = CGSizeMake(0, 1);//(0,5)
    self.layer.shadowOpacity = 0.5;//1.0
    self.layer.shadowRadius = 2.0;//10.0
...
-(void) touchUp: (UIButton*) sender {
    [UIView animateWithDuration:0.01 animations:^{
        ...
        CATransform3D transform = CATransform3DMakeRotation(0, 0.1, 0, 0);
        ...
        [((MTMetroButton*)sender) enableShadow];
...
-(void) touchDown: (UIButton*) sender forEvent:(UIEvent*)event{
    ...
    CGFloat a = (sender.frame.size.width / 2.0f) - location.x;//2.0f
    CGFloat b = (sender.frame.size.height / 2.0f) - location.y;//2.0f
    ...
    if (fabs(a) < sender.frame.size.width / 4.0f && fabs(b) < sender.frame.size.height / 4.0f) {
        ...
        [UIView animateWithDuration:0.01 animations:^{
            sender.layer.zPosition = 10000;
            sender.layer.frame = CGRectMake(sender.layer.frame.origin.x, sender.layer.frame.origin.y, originalSize.width * 0.99, originalSize.height * 0.99);//0.8
            sender.center = originalCenter;
            [((MTMetroButton*)sender) disableShadow];
        } completion:^(BOOL finished) {
    ...
    if (fabs(a) > fabs(b))
        if (a < 0) //left side
            transform3d = CATransform3DMakePerspective(0.000005,0 );//0.0005
        else //right side
            transform3d = CATransform3DMakePerspective(-0.000005, 0);
    //y distance is greater
        else
            if (b < 0) //top
                transform3d = CATransform3DMakePerspective(0 , 0.000005);
            else //bottom
                transform3d = CATransform3DMakePerspective(0 , -0.000005);
